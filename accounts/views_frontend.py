    ...
    if request.method == "POST":
        ...
        if rol == "medico":
            if not numero_matricula:
                errors["numero_matricula"] = _(
                    "El número de matrícula es obligatorio para médicos."
                )
...
def dashboard(request):
    ...
    if u.rol == "paciente":
        ...
        if medico:
            ...
            ctx["proximas"] = medico.citas.filter(
                horario__fecha__gt=today, estado__nombre__in=["pendiente", "confirmada"]
            ).count()
            ctx["pacientes_atendidos"] = (
                medico.citas.filter(estado__nombre="realizada")
                .values("paciente")
                .distinct()
                .count()
            )
            ctx["citas_recientes"] = medico.citas.select_related(
        ...
        ctx["citas_recientes"] = Cita.objects.select_related(
            "paciente__usuario", "medico__usuario", "horario", "estado"
        ).order_by("-horario__fecha", "-horario__hora_inicio")[:5]
