...
@extend_schema(
    tags=["Auth"], summary="Registrar usuario", request=RegisterSerializer, responses={201: None}
)
@api_view(["POST"])
...
@extend_schema(
    tags=["Auth"], summary="Iniciar sesión", request=LoginSerializer, responses={200: None}
)
@api_view(["POST"])
@permission_classes([AllowAny])
def login(request):
...
@extend_schema(
    tags=["Auth"],
    summary="Obtener / actualizar perfil",
