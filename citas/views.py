...
def cita_detail(request, pk):
    ...
    try:
        cita = Cita.objects.select_related("paciente", "medico", "horario", "estado").get(pk=pk)
    ...
    if request.method == "GET":
        ...
        can_edit = request.user.is_staff or cita.paciente.usuario == request.user
        if not can_edit:
            return Response(
...
def cancelar_cita(request, pk):
    ...
    cita_datetime = (
        timezone.make_aware(cita_datetime) if timezone.is_naive(cita_datetime) else cita_datetime
    )
    now = timezone.now()
